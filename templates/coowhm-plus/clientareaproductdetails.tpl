            ...
            <div class="product-details clearfix">
                ...
                <div class="row">
                    <div class="col-md-6">
                        ...
                        <div class="prodectsdetails-overview-box {$rawstatus|strtolower}">
                            <div class="prod-info-abso">
					...
					<div class="overview-box products-details">
                        <span>{$LANG.clientareahostingregdate}<strong>{$regdate}</strong></span>

                        {if $firstpaymentamount neq $recurringamount}
                            <span>{$LANG.firstpaymentamount}<strong>{$firstpaymentamount}</strong></span>
                        {/if}

                        {if $billingcycle != $LANG.orderpaymenttermonetime && $billingcycle != $LANG.orderfree}
                            <span>{$LANG.recurringamount}<strong>{$recurringamount}</strong></span>
                        {/if}

                        <span>{$LANG.orderbillingcycle}<strong>{$billingcycle}</strong></span>

                        <span>{$LANG.clientareahostingnextduedate}<strong>{$nextduedate}</strong></span>

                        <span>{$LANG.orderpaymentmethod}<strong>{$paymentmethod}</strong></span>
