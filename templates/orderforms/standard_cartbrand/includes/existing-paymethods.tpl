{if $selectedAccountId === $client->id}
    {if $isMultibrandActive}
        {assign "multibrandPayMethods" $client->payMethods->validateGateways()->sortByExpiryDate()|multibrandGateways}
    {else}
        {assign "multibrandPayMethods" $client->payMethods->validateGateways()->sortByExpiryDate()}
    {/if}
    {foreach $multibrandPayMethods as $payMethod}
