...
{elseif isset($RSThemes['pages'][$templatefile]) && file_exists($RSThemes['pages'][$templatefile]['fullPath'])}
                ...
                <form method="post" action="domainchecker.php" id="frmDomainHomepage">
                    ...
                    <div class="domain-search-input search-group search-group-combined">
                        <div class="search-field search-field-lg">
                            ...
                            <input class="form-control form-control-lg" type="text" name="domain" placeholder="{$LANG.exampledomain}" autocapitalize="none" data-toggle="tooltip" {if ($language == 'arabic' || $language == 'hebrew' || $language == 'farsi')} data-placement="right" {else} data-placement="left" {/if} data-trigger="manual" title="{lang key='orderForm.required'}"/>
                        </div>
                        <div class="search-group-btn">
                            {if $transferdomainenabled}
                                <button type="submit" class="btn btn-lg btn-primary-faded transfer {$captcha->getButtonClass($captchaForm)}" data-domain-action="transfer">
                                    {$LANG.domainstransfer}
                                </button>
                            {/if}
                            {if $registerdomainenabled}
                                <button type="submit" class="btn btn-lg btn-primary search {$captcha->getButtonClass($captchaForm)}" id="btnDomainSearch">
                                    {$LANG.search}
                                </button>
                            {/if}
                        </div>
