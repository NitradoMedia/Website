...
{else}
    ...
    {if $unpaidInvoice}
        <div class="alert alert-lagom alert-{if $unpaidInvoiceOverdue}danger{else}warning{/if}" id="alert{if $unpaidInvoiceOverdue}Overdue{else}Unpaid{/if}Invoice">
    ...
    <div class="tab-content margin-bottom">
        <div class="tab-pane active" id="Overview">
            ...
            {else}
                ...
                {/foreach}
                {if $domain || $configurableoptions || $customfields || $customModuleInfo}
                <div class="section">
                    <div class="section-body">
                        <div class="panel panel-default">
                            <div class="panel-nav">
                                <ul class="nav nav-tabs">							
                                    {if $customModuleInfo || $lastupdate}
                                    <li>
                                    ...
                                    {if $domain}
                                        <li>
                                            <a class="{if !$customModuleInfo && !$lastupdate}active{/if}" href="#domain" data-toggle="tab"><i class="ls ls-location"></i> {if $type eq "server"}{$LANG.sslserverinfo}{elseif ($type eq "hostingaccount" || $type eq "reselleraccount") && $serverdata}{$LANG.hostingInfo}{else}{$LANG.clientareahostingdomain}{/if}</a>
                                        </li>
                                    {/if}
                                    {if $configurableoptions}
                                        <li>
                                            <a class="{if !$domain && !$moduleclientarea}  active{/if}" href="#configoptions" data-toggle="tab"><i class="ls ls-configure"></i> {$LANG.orderconfigpackage}</a>
                                        </li>
                                    {/if}
                                    {if $metricStats}
                                        <li>
                                            <a class="{if !$domain && !$moduleclientarea && !$configurableoptions} active{/if}"  href="#metrics" data-toggle="tab"><i class="fas fa-chart-line fa-fw"></i> {$LANG.metrics.title}</a>
                                        </li>
                                    {/if}
                                    {if $customfields}
                                        <li>
                                            <a class="{if !$domain && !$moduleclientarea && !$metricStats && !$configurableoptions} active{/if}" href="#additionalinfo" data-toggle="tab"><i class="ls ls-info-circle"></i> {$LANG.additionalInfo}</a>
                                        </li>
                                    {/if}
                                </ul>
                            </div>               
                            <div class="tab-content">							
                                {if $customModuleInfo || $lastupdate}
                                    <div class="panel-body tab-pane{if $customModuleInfo || $lastupdate} active{/if} billingOverview" id="billingInfo">
                                        <div class="row row-eq-height row-eq-height-sm ">				
                                            {if $firstpaymentamount neq $recurringamount}
                                ...
                                {/if}
                                {if $domain}
                                <div class="tab-pane {if !$customModuleInfo && !$lastupdate}active{/if}" id="domain">
                                ...
                                {/if}
                                {if $configurableoptions}
                                <div class="tab-pane {if !$domain && !$moduleclientarea} active{/if}" id="configoptions">
                                    <ul class="list-info list-info-50 list-info-bordered">
                                ...
                                {/if}
                                {if $metricStats}
                                    <div class="tab-pane {if !$domain && !$moduleclientarea && !$configurableoptions}active{/if}" id="metrics">
                                        {include file="$template/clientareaproductusagebilling.tpl"}
                                    </div>
                                {/if}
                                {if $customfields}
                                <div class="tab-pane {if !$domain && !$moduleclientarea && !$configurableoptions && !$metricStats} active{/if} " id="additionalinfo">
                                    <ul class="list-info list-info-50 list-info-bordered">
                                ...
                                <div class="tab-pane fade text-center" id="ssl-info">
                                    {if $sslInfo->active}
                                        <div class="alert alert-lagom alert-success" role="alert">
                                            {lang key='sslActive' expiry=$sslInfo->expiryDate->toClientDateFormat()}
                                        </div>
                                    {else}
                                        <div class="alert alert-lagom alert-warning ssl-required" role="alert">
        ...
        <div class="tab-pane " id="Downloads">
            <div class="section">
                <div class="section-header">
                    <h2 class="section-title">{$LANG.downloadstitle}</h2>
                </div>
                <div class="section-body">
                    <p class="text-light">{lang key="clientAreaProductDownloadsAvailable"}</p>
