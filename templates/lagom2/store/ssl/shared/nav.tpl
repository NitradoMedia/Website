{if file_exists("templates/$template/store/ssl/shared/overwrites/nav.tpl")}
    {include file="{$template}/store/ssl/shared/overwrites/nav.tpl"}  
{else}
    ...
    <div class="container">
      ...
      <div class="collapse navbar-collapse" id="nav-ssl">
        <ul class="nav nav-tabs">
          <li {if $current == 'index'} class="active"{/if}><a href="{routePath('store-product-group', $routePathSlug)}">{lang key='store.websiteBuilder.tab.overview'}</a></li>
