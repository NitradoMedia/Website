                    ...
                    {foreach $item->getChildren() as $childItem}
                        {if $childItem->getUri()}
                            <a menuItemName="{$childItem->getName()}" 
                                href="{if $item->getName() =="Domain Details Management" || $item->getName() =="Service Details Overview"}{if $childItem->getUri()|strstr:"action"}{$childItem->getUri()}{else}{$childItem->getUri()|replace:$WEB_ROOT:""|replace:"/":""}{/if}{else}{$childItem->getUri()}{/if}" 
