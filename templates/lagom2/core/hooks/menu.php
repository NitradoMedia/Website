...
function getDisplayRule($pageName) {
    ...
    if ($display->name == 'Default') {
        if (isset($_SESSION['uid']) || (isset($_SESSION['login_auth_tk']) && !is_null($_SESSION['login_auth_tk']))) {
                        ...
                        if (!$divider) {
                            ...
                            if ($child->content->type->name == 'WHMCS Page') {
                                ...
                                if ($childPage) {
                                    ...
                                    if ($childDescription && $parent->content->display_settings['dropdown'] == "extended"){
                                        $template = '<span class="menu-item-content">'. $childLabel . '<span class="menu-item-desc">'. $childDescription .'</span></span>'; 
                                    }
                                    else{
                                        $template = $childLabel;
                                    }
                                    ...
                                    if ($list == 'Client Details') {                                      
                                        ...
                                        if (isset($vars['loggedinuser']) && $vars['loggedinuser']->first_name){
                                            ...
                                            $template .= '                                               
                                                ...
                                                '. $vars['loggedinuser']->first_name .' ' . $vars['loggedinuser']->last_name .'
                                                </div>
                                                ';
                            ...
                            if (isset($child->content->display_settings['style'])) {
                                ...
                                if (isset($childItem) && !empty($childItemStyle)) {
                                    ...
                                    if ((!isset($child->content->content_settings['icon']) && !isset($child->content->content_settings['predefined_icon'])) || (isset($child->content->content_settings['icon']) && empty($child->content->content_settings['icon']) && isset($child->content->content_settings['predefined_icon']) && empty($child->content->content_settings['predefined_icon']))){
                                        if ($childItemStyle == 'text'){
                                            $childBuildInClasses .= 'nav-item-text-only';
                                        }
                                    }
